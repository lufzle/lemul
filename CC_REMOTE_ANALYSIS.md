...
- [x] **Onboarding runbook for Bedrock model access** — [`docs/onboarding-bedrock.md`](docs/onboarding-bedrock.md). Covers the FTU form (inheritance from the Org management account, instant grant), example `useCases` wording, the **denial path with no self-service recovery**, the failure-to-fix mapping, why pins must be inference profile IDs, and the two traps (temporary credentials, VPC endpoint). Ordered so the multi-day failure is cleared first.
