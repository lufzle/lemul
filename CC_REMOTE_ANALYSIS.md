...
- [ ] Supervisor — **multi-PTY manager** (one per session, §2.3), resize channel, replay buffer, idle reporter, resource-headroom reporter; survives client disconnect
...
2. [ ] Reconnect/replay path hardened — one mechanism, three consumers: network-drop recovery, relay redeploy, and (later) vertical migration (§2.4, §2.8).
...
| 4 | Reconnect: tmux or own VT state model? | **tmux for Phase 1**, structure supervisor so a VT model can replace it in Phase 2 | tmux ships this week but normalizes `TERM` to `tmux-256color` and taxes true color, OSC 52, and the enhanced keyboard protocol — exactly the features that make CC feel good. **Join-as-Viewer (§2.5) forces this decision** — showing a late joiner the current screen needs server-side screen state, which a replay buffer cannot provide. **Now on the Phase 1 critical path:** E2E rules out relay-side screen state (§2.7), so the model must live in the supervisor — which shapes how the supervisor is written from the first commit. |
