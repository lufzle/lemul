...
func TestStopOfAnUnknownSessionIs404(t *testing.T) {
	...
	code, _ := s.post("/v1/sessions/00000000-0000-4000-8000-000000000000/stop")
...
func TestResumeStartsTheProcessWithoutAnyClient(t *testing.T) {
	...
	if code, body := s.post("/v1/sessions/" + sid + "/resume"); code != http.StatusOK {
...
func TestDeleteRefusesARunningSessionWithoutForce(t *testing.T) {
	...
	code, body := s.del("/v1/sessions/" + sid)
	...
	if code, body := s.del("/v1/sessions/" + sid + "?force=1"); code != http.StatusNoContent {
...
func TestDeleteAStoppedSessionAndThenAgain(t *testing.T) {
	...
	if code, body := s.del("/v1/sessions/" + sid); code != http.StatusNoContent {
		t.Fatalf("delete: %d: %s", code, body)
	}
	if code, _ := s.del("/v1/sessions/" + sid); code != http.StatusNoContent {
