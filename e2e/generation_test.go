...
type workspaceDocT struct {
	...
	// Section 2.4's cascade, read by idle_test.go. Both are absent on a
	// workspace in neither state, which is why they are plain strings: an
	// empty one means "not in it" rather than a zero time.
	WarmHoldUntil   string `json:"warm_hold_until"`
	IdlePinnedSince string `json:"idle_pinned_since"`
	// LastError is why the last placement failed (increment 4). Placement is a
	// background operation now, so this is the only thing that reports one --
	// the request that would have carried it was answered 202 long before.
	LastError string `json:"last_error"`
}

func (s *stack) workspaces() []workspaceDocT {
	s.t.Helper()
	resp, err := s.hGet(s.orgURL + "/workspaces")
	if err != nil {
