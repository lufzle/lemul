...
func TestExplorerListsTheWorkspace(t *testing.T) {
	...
	var got dirListing
	code, body := s.getJSON(t, "/v1/workspaces/"+wid+"/fs?path=/", &got)
	...
	code, body = s.getJSON(t, "/v1/workspaces/"+wid+"/fs?path=/src", &got)
	if code != http.StatusOK {
...
func TestExplorerRefusesToLeaveTheWorkspace(t *testing.T) {
	...
	code, body := s.getJSON(t, "/v1/workspaces/"+wid+"/fs?path="+url.QueryEscape("/etc"), nil)
	if code == http.StatusOK {
...
func TestExplorerAttributesProcessesToSessions(t *testing.T) {
	...
	code, body := s.getJSON(t, "/v1/workspaces/"+wid+"/processes", &got)
	if code != http.StatusOK {
...
func TestExplorerResourcesReportAvailability(t *testing.T) {
	...
	code, body := s.getJSON(t, "/v1/workspaces/"+wid+"/resources", &got)
	if code != http.StatusOK {
...
func TestExplorerDoesNotStartAWorkspace(t *testing.T) {
	...
	code, _ := s.getJSON(t, "/v1/workspaces/never-started/fs?path=/", nil)
	if code == http.StatusOK {
