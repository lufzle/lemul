// Package tunnel carries the runner<->relay connection.
//
// Two pieces, both needed because a yamux stream is a raw byte pipe:
//
//   - wsnet adapts a WebSocket into a net.Conn so yamux can multiplex over it.
//     The runner dials OUT to the relay and holds this open; every user session
//     becomes a stream on it. No ingress into the customer VPC.
//
