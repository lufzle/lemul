...
export const Route = createFileRoute('/_authenticated/orgs/$org/workspaces/$wid_/sessions/$sid')({
  loader: async ({ params }) => {
    ...
    const sessions = await listSessions({ data: { org: params.org, wid: params.wid } })
    const session = sessions.find((s) => s.id === params.sid)
    if (!session) throw notFound()
    return { org: params.org, wid: params.wid, session }
  },
...
function SessionViewer() {
  const { org, wid, session } = Route.useLoaderData()
      ...
      {isRunning ? (
        <p className="text-xs text-neutral-500">
          ...
          <code className="text-neutral-300">
            lem --org {org} connect {wid} -session {session.id}
          </code>
        </p>
      ) : null}
    </div>
