...
type Token struct {
	...
	// IDToken is relayed ONCE, at login, to PUT /v1/identity -- it is the only
	// thing that carries an email, and the control plane will not take one it
	// cannot verify. Deliberately not refreshed alongside the access token: it
	// is an assertion about a sign-in that happened, not a credential, and a
	// stale one is simply not sent again.
...
var ErrNotSignedIn = errors.New("not signed in: run `lem login`")
...
func Save(t *Token) error {
	...
	// Serialising the token is the point: `lem` has to keep it between
	// invocations or every command would restart the device flow. It is written
	// 0600 into a 0700 directory a few lines below, which is the control that
	// applies -- gosec is flagging the field name, not a mishandling of it.
	// #nosec G117 -- persisting the token is the feature; the file mode is the guard
