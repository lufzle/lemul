...
type tokenResponse struct {
	...
	// IDToken is what carries identity. The access token is minted for an API
	// resource and has a subject and no identity claims at all, so an email can
	// only arrive as a signed assertion -- which this is. It was requested from
	// the start (the scope below asks for `openid email`) and simply never
	// parsed, so nothing had ever looked to see whether it came back.
