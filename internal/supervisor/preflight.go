...
func (s *Supervisor) runPreflight(ctx context.Context) tunnel.PreflightReport {
	...
	if !s.opt.BedrockPreflight {
		log.Printf("bedrock preflight skipped (workspace is not using bedrock)")
		rep.Skipped = true
		return rep
	}
	...
	if err != nil {
		...
		log.Printf("bedrock preflight: load AWS config: %v", err)
		rep.Blocking = true
	...
	if rep.Blocking {
		log.Printf("bedrock preflight FAILED for workspace %s:\n%s", s.opt.WorkspaceID, res)
	} else {
		log.Printf("bedrock preflight ok for workspace %s (%s)", s.opt.WorkspaceID, res.Region)
	}
	return rep
}
