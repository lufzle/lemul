...
// proxy signs whatever it forwards. Without an allowlist a Bash command inside
// the sandbox directs a fully authenticated request at any route the gateway
// exposes -- on LiteLLM, key minting and every tenant's spend history.
func TestProxyForwardsOnlyInferenceRoutes(t *testing.T) {
	var reached []string
	...
	b, err := New(Options{Upstream: upstream.URL, APIKey: "sk-real", WorkspaceID: "w1"})
	if err != nil {
		t.Fatal(err)
	}
	front := httptest.NewServer(b.handler(&SessionProxy{SessionID: "s1"}))
	defer front.Close()
	...
	for _, tc := range []struct {
		...
		// The routes that make a leaked capability expensive.
		{"POST", "/key/generate", 404},
		{"GET", "/spend/logs", 404},
		{"GET", "/v1/models", 404}, // discovery off by default
		{"GET", "/", 404},
		{"POST", "/v1/chat/completions", 404},
		// Method matters: a GET on the inference route is not inference.
		{"GET", "/v1/messages", 404},
		// Dot segments must not smuggle a denied route past a literal match.
		// Go does not resolve these for a bare handler, so the proxy has to.
		{"POST", "/v1/messages/../key/generate", 404},
		{"POST", "/v1/messages/../../key/generate", 404},
	} {
	...
	for _, got := range reached {
		if got != "POST /v1/messages" && got != "POST /v1/messages/count_tokens" {
			t.Errorf("a denied request reached the gateway carrying the real key: %s", got)
		}
	}
}

// The check and the forward must agree on the path.
//
// Deciding on one string and sending another is how an allowlist gets walked
// past: the proxy would authorise "/v1/messages" and the gateway would route
// whatever it was actually handed. So the cleaned path is both what `allows`
// sees and what goes upstream.
func TestProxyForwardsThePathItChecked(t *testing.T) {
	...
	b, err := New(Options{Upstream: upstream.URL, APIKey: "sk-real"})
	if err != nil {
		t.Fatal(err)
	}
	front := httptest.NewServer(b.handler(&SessionProxy{SessionID: "s1"}))
	...
	// Each of these denotes /v1/messages, and each must arrive as /v1/messages
	// rather than in whatever form it was written.
	for _, raw := range []string{
		...
		req, err := http.NewRequest("POST", front.URL+raw, nil)
		if err != nil {
		...
		if resp.StatusCode != http.StatusOK {
			t.Errorf("%s = %d, want 200", raw, resp.StatusCode)
			continue
		}
		if arrived := <-got; arrived != "/v1/messages" {
			t.Errorf("%s arrived upstream as %q; the gateway would route something "+
				"other than what was authorised", raw, arrived)
		}
	}
}

// Discovery is opt-in on the same variable that turns on Claude Code's own
// gateway model discovery, so the setting and the route it needs cannot drift.
func TestModelDiscoveryIsOptIn(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer upstream.Close()

	b, err := New(Options{Upstream: upstream.URL, APIKey: "sk-real", AllowModelDiscovery: true})
	if err != nil {
		t.Fatal(err)
	}
	front := httptest.NewServer(b.handler(&SessionProxy{SessionID: "s1"}))
	...
	for path, want := range map[string]int{
		"/v1/models":            200,
		"/v1/models/claude-4-5": 200,
		"/spend/logs":           404, // still closed
	} {
