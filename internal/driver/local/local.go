...
func (d *Driver) Start(ctx context.Context, s driver.Spec) (string, error) {
	...
	// Not CommandContext: a workspace task outlives the request that placed it,
	// exactly as a Fargate task does. Tying it to the dispatch context would make
	// the local driver behave unlike the one it stands in for.
	//nolint:noctx // the task's lifetime is the workspace's, not a request's
