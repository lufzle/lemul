...
// UTF-8 validation, which corrupts it. Binary frames carry PTY bytes as-is.
//
// This distinction does NOT survive the tunnel to the supervisor, which is a
// raw byte pipe; internal/tunnel re-establishes it with an explicit type tag.
package proto
...
// only one concurrent writer, and both sides have two writing goroutines
// (byte pump + resize handler).
type Conn struct {
	ws *websocket.Conn
	mu sync.Mutex
}
...
// connection (abnormal/no close frame -- reconnect and replay).
