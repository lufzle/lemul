...
type Spec struct {
	...
	Rows uint16
	Cols uint16
	// Dir is the working directory the process starts in. Empty inherits this
	// process's, which is the local driver and the e2e suite.
	//
	// It is set here rather than through a setting, because a workspace is
	// shared and each member starts in their own home: Claude Code's own
	// configuration is inside that home and therefore writable by the person it
	// would be constraining. The fork is the only place this can be decided by
	// somebody other than the session.
	Dir string
	// Credential, when set, runs the child at that uid/gid rather than the
	// supervisor's own.
	//
	// This is a security boundary, not tidiness. Stripping the gateway key from
	// the child's environment (supervisor.strippedFromChild) removes the
	// variable but not the secret: the supervisor still holds it, and a child at
	// the same uid in the same PID namespace reads it straight back out of
	// /proc/<supervisor>/environ. Measured -- see section 12.6.
	//
	// CLAUDE_CODE_SUBPROCESS_ENV_SCRUB closes that with a PID namespace, but it
	// needs bubblewrap and seccomp privileges Fargate does not grant, so the
	// boundary has to be one the platform allows. A uid difference is: the
	// environ files are mode 0400 owned by the supervisor's uid, so a different
	// uid simply cannot open them.
...
type Session struct {
	...
	exited      bool
	exitCode    int
	// lastOutput is when the PTY last produced bytes -- section 2.4's second
	// idle condition. Recorded under the same lock as the ring push, so it
	// cannot disagree with what was actually delivered.
...
func newSession(spec Spec, ringBytes int, nudgeDelay time.Duration) (*Session, error) {
	...
	// Deliberately NOT CommandContext. Binding the child to a context is exactly
	// the bug this package exists to prevent: the PTY would die with whatever
	// request created it, and "the client disconnected" would once again mean
	// "the agent stopped". Detach is the child outliving its connection.
	//nolint:noctx // the child's lifetime is the session's, not a request's
	c := exec.Command(spec.Cmd[0], spec.Cmd[1:]...)
	c.Env = spec.Env
	c.Dir = spec.Dir
	if spec.Credential != nil {
		// StartWithSize preserves an existing SysProcAttr and only adds Setsid
		// and Setctty to it, so setting the credential here survives the fork.
		c.SysProcAttr = &syscall.SysProcAttr{Credential: spec.Credential}
	}
	...
	s := &Session{
		...
		rows:        spec.Rows,
		cols:        spec.Cols,
		// Seeded to the start rather than left zero: "has produced nothing yet"
		// and "has been silent since it started" are the same fact, and a zero
		// time would read as idle since 1970.
		lastOutput: now,
...
//
// Measured against Claude Code 2.1.220: the repaint homes the
...
// LastOutput is when the PTY last produced bytes, for section 2.4's idle
// detection. Never zero: a session that has printed nothing reports its own
// start time, so "silent since it began" is a duration rather than a special
